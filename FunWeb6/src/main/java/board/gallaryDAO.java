...
//		 return con;
	...
	public void ginsertBoard(BoardBean bb) {
		Connection con=null;
		...
		try {
			...
			con=getConnection();
			 ...
			 // => 이번에 입력할 글 번호 구하기
			 int num=0;
			 String sql2="select max(num) from gallary";
			 ...
			 String sql="insert into gallary(num , name, pass, subject, content, readcount, date, file) values(?,?,?,?,?,?,?,?)";
	...
	public List ggetBoardList(int startRow,int pageSize) {
		Connection con=null;
		PreparedStatement pstmt2=null;
		ResultSet rs=null;
		PreparedStatement pstmt=null;
...
//			 String sql="select * from board order by num desc limit 시작하는행-1,한화면에 보여줄 글개수";
			 String sql="select * from gallary order by num limit ?,?";
			 pstmt=con.prepareStatement(sql);
	...
	public int ggetBoardCount() {
		...
		try {
			...
			con=getConnection();
			// 3sql 
			String sql="select count(*) from gallary";
			pstmt=con.prepareStatement(sql);
			// 4실행 => 결과저장
			rs=pstmt.executeQuery();
			// 5 결과 다음행으로 이동 열가져오기
	...
	public BoardBean ggetBoard(int num) {
		Connection con=null;
		PreparedStatement pstmt2=null;
		ResultSet rs=null;
		PreparedStatement pstmt=null;
	...
	public void gupdateReadcount(int num) {
		Connection con=null;
		PreparedStatement pstmt2=null;
		ResultSet rs=null;
		PreparedStatement pstmt=null;
